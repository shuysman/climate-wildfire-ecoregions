...
for ECOREGION in $ENABLED_ECOREGIONS; do
  ...
  if Rscript "$PROJECT_DIR/src/operational/html_generation/generate_daily_html.R" "$ECOREGION"; then
...
if Rscript "$PROJECT_DIR/src/operational/html_generation/generate_index_html.R"; then

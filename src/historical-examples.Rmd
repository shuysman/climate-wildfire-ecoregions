...
non_forest_data_full <- terra::roll(vpd_data, n = 3, fun = mean, type = "to", circular = FALSE, overwrite = TRUE)
...
generate_and_plot_fire_danger <- function(dates, plot_title) {
  ...
  forest_fire_danger_rast <- resample(forest_fire_danger_rast, classified_rast, method="near")
  non_forest_fire_danger_rast <- resample(non_forest_fire_danger_rast, classified_rast, method="near")
...
generate_and_plot_fire_danger(
  ...
  plot_title = "Fire Danger: June 1988 (Drought)"
)
```
...
```{r}
generate_and_plot_fire_danger(
  dates = as_date("2011-06-04") + 0:7,
  plot_title = "Fire Danger: June 2011 (Wet)"
)
```

...
knitr::opts_chunk$set(echo = F, warning = F, message = F, fig.height = 12, fig.width = 16)
```
...
forest_data_full <- terra::roll(vpd_data, n = 5, fun = mean, type = "to", circular = FALSE, overwrite = TRUE)
non_forest_data_full <- terra::roll(vpd_data, n = 21, fun = mean, type = "to", circular = FALSE, overwrite = TRUE)
...
generate_and_plot_fire_danger <- function(dates, plot_title) {
  ...
  p <- ggplot() +
    ...
    labs(title = plot_title, fill = "Fire Danger")
...
generate_and_plot_fire_danger(
  dates = as_date("1988-07-05") + 0:7,
  plot_title = "Fire Danger: August 1988 (Drought)"
)
```
...
```{r}
generate_and_plot_fire_danger(
  dates = as_date("2016-08-15") + 0:7,
  plot_title = "Fire Danger: Mid-August 2016 (Drought)"
)
```
...
```{r}
generate_and_plot_fire_danger(
  dates = as_date("2011-07-20") + 0:7,
  plot_title = "Fire Danger: Late July 2011 (Wet)"
)
```
...
```{r}
generate_and_plot_fire_danger(
  dates = as_date("1993-07-20") + 0:7,
  plot_title = "Fire Danger: Late July 1993 (Wet)"
)
```

...
api_url <- glue("https://api.weatherbit.io/v2.0/history/lightning?lat=43.5459517032319&lon=-111.162554452619&end_lat=45.1292422224309&end_lon=-109.829085745439&date={today}&key={api_key}")
...
if (!is.null(lightning_data) && !is.null(lightning_data$lightning) && is.data.frame(lightning_data$lightning) && nrow(lightning_data$lightning) > 0) {
  ...
  header_content <- paste(header_title, update_time, lightning_table)
} else {
  no_strikes_message <- "<p>No lightning strikes recorded today.</p>"
  header_content <- paste(header_title, update_time, no_strikes_message)
}
...
saveWidget(m, file.path(out_dir, glue("lightning_map_{today}.html")), selfcontained = TRUE)
